...
try {
    ...
    $totalEarning = 0;
    $appointments = [];  // Store appointments to avoid re-executing the query

    while ($user_details = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $appointments[] = $user_details;
        $totalEarning += ($user_details['cost'] * 0.9);
    }
...
// Delete appointment if necessary
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive mt-4">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                <?php foreach ($appointments as $user_details): ?>
                                    <tr>
                                        <td><?php echo $user_details['name']; ?></td>
                                        <td><?php echo date('g:i A', strtotime($user_details['appointment_time'])); ?></td>
                                        <td><?php echo date('j F, Y', strtotime($user_details['date'])); ?></td>
                                        <td><?php echo ($user_details['cost'] * 0.9); ?></td>
                                    </tr>
                                <?php endforeach; ?>
